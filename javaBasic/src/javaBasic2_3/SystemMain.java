...
public class SystemMain {
	...
	public static void main(String[] args) {
		...
		System.out.printf("자바 버전:%s\n", System.getProperty("java.version"));
		
		//자바로 응용 프로그램을 만들면 자바가 설치되어 있어야만
		//실행이 가능하므로 이 프로퍼티의 값을 확인해서
		//값이 없으면 자바를 설치하라고 해야 합니다.
		
		//path 라는 환경 변수의 값을 출력 ----------------> 엄청 중요 ㅇ
		//getenv 라는 메소드 이용
		...
		System.out.printf("걸린 시간1:%d\n", (end-start));
		
		start = System.currentTimeMillis();
		for(int i=0; i<1000000000; i=i+1);
		end = System.currentTimeMillis();
		System.out.printf("걸린 시간2:%d\n", (end-start));
